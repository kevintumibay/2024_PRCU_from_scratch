...
// High pressure sensor variables
// FILL OUT
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		...
		/*
		// Check if CO2 sensor is wired correctly
		...
		if (HAL_I2C_Master_Transmit(&hi2c2, CO2_sensor_ADDR, CO2_sensor_data_ready, 2, 1) == HAL_OK) {
			if (HAL_I2C_Master_Receive(&hi2c2, CO2_sensor_ADDR | 0x01, CO2_buffer, 3, 50) == HAL_OK) {
				printf("CO2 sensor status = %i - %i - %i \r\n", (int)CO2_buffer[0], (int)CO2_buffer[1], (int)CO2_buffer[2]);
			}
			else{
			...
			printf("CO2 sensor NOT TRANSMITTING \r\n");
		}
